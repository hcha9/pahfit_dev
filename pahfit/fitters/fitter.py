...
class Fitter(ABC):
    ...
    ensures that the function signatures are the same between different 
    Fitter implementations, so that the :class:pahfit.fitter.Fitter setup 
    can remain generic (in practice, this is a loop over the Features table
    ...
    @abstractmethod
    def fit(self, lam, flux, unc, maxiter=1000, method=None, **fit_kwargs):
